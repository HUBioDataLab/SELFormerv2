...
class MultiModalTransformers_For_Classification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, text_embeddings, unimol_embeddings, attention_mask, labels):
        
        outputs = self.roberta(input_ids, attention_mask=attention_mask)
        sequence_output = outputs[0]
        
        # We can concatenate, sum, or average the embeddings
        
        full_embeddings = torch.cat((sequence_output, text_embeddings, unimol_embeddings), dim=1)
...
class SELFIESTransfomers_Dataset(Dataset):
    def __init__(self, data, MAX_LEN):
        text, desc, unimol, labels = data
        self.examples = tokenizer(text=text, text_pair=None, truncation=True, padding="max_length", max_length=MAX_LEN, return_tensors="pt")
        self.desc = torch.tensor(desc, dtype=torch.float)
        self.unimol = torch.tensor(unimol, dtype=torch.float)
    ...
    def __getitem__(self, index):
        ...
        item["desc"] = self.desc[index]
        item["unimol"] = self.unimol[index]
...
if args.use_scaffold == 0:  # random split
    ...
    train_df = pd.DataFrame(np.column_stack([train_smiles, train.targets()]), columns=["sequence_embeddings", "text_embeddings", "unimol_embeddings," "target"])
    validation_df = pd.DataFrame(np.column_stack([validation_smiles, val.targets()]), columns=["sequence_embeddings", "text_embeddings", "unimol_embeddings," "target"])
    test_df = pd.DataFrame(np.column_stack([test_smiles, test.targets()]), columns=["sequence_embeddings", "text_embeddings", "unimol_embeddings," "target"])


test_y = pd.DataFrame(test_df.target, columns=["target"])

MAX_LEN = 128
train_examples = (train_df.iloc[:, 0].astype(str).tolist(), train_df.iloc[:, 1].tolist(), train_df.iloc[:, 2].tolist(), train_df.iloc[:, 3].tolist())
train_dataset = SELFIESTransfomers_Dataset(train_examples, MAX_LEN)

validation_examples = (validation_df.iloc[:, 0].astype(str).tolist(), validation_df.iloc[:, 1].tolist(), validation_df.iloc[:, 2].tolist(), validation_df.iloc[:, 3].tolist())
validation_dataset = SELFIESTransfomers_Dataset(validation_examples, MAX_LEN)

test_examples = (test_df.iloc[:, 0].astype(str).tolist(), test_df.iloc[:, 1].tolist(), test_df.iloc[:, 2].tolist(), test_df.iloc[:, 3].tolist())
test_dataset = SELFIESTransfomers_Dataset(test_examples, MAX_LEN)
